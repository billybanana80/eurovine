...
#   Ozivine: U (formerly UKTV Play) Video Downloader
#   Author: billybanana
#   Usage: enter the series/season/episode URL to retrieve the MPD, Licence, PSSH and Decryption keys.
#   eg: https://u.co.uk/shows/sisters/series-1/episode-1/6355804300112
#   Authentication: None
#   Geo-Locking: requires a UK IP address
#   Quality: up to 1080p
#   Key Features:
#   1. Extract Video ID: Parses the U URL and queries the page source code to extract the series name, season, and episode number.
...
#   5. Note: this script functions for encrypted and non-encrypted video files
...
def build_download_command(manifest_url, source_type, save_name, keys=None, interactive=False):
...
def resolve_playback(video_url, interactive=False):
    ...
    return {
        ...
        "encrypted": encrypted,
        "save_name": save_name,
        "download_command": build_download_command(manifest_url, manifest_type, save_name, keys, interactive=interactive),
    }
...
def get_download_command(video_url, auto_download=False, interactive=False):
    ...
    spinner.start()
    try:
        playback = resolve_playback(video_url, interactive=interactive)
    except Exception:
        spinner.stop()
        raise
    spinner.stop()
