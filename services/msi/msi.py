...
SERVICE_KEY = "msi"
SERVICE_NAME = "Mediaset Infinity IT"
SERVICE_TAG = "MSI"
SERVICE_DISPLAY_NAME = SERVICE_NAME
SERVICE_URL_PREFIXES = ("https://mediasetinfinity.mediaset.it",)
...
APP_NAME = "embed//mediasetplay-embed/2.24.0-c873f5a"
...
def extract_ids_from_url(input_url):
    ...
    parsed = urlparse(url)
    if not parsed.netloc.lower().endswith("mediaset.it"):
        raise ValueError("Expected a Mediaset Infinity URL.")

    path = parsed.path
    return {
        "url": base_url_without_query(url),
        "series_id": next(iter(re.findall(r"(SE\d+)", path)), ""),
        "season_id": next(iter(re.findall(r"(ST\d+)", path)), ""),
        "subbrand_id": next(iter(re.findall(r"sb(\d+)", path, flags=re.I)), ""),
        "video_id": next(iter(re.findall(r"_([A-Z]\d+[A-Z0-9]*)", path)), ""),
    }


def series_slug_from_url(input_url):
    parsed = urlparse(canonical_url(input_url))
    parts = [part for part in parsed.path.split("/") if part]
    if len(parts) >= 2 and parts[0] in {"fiction", "programmi-tv", "serie-tv", "show"}:
...
def parse_msi_date(value):
    ...
    for date_format in (
        ...
        try:
            return datetime.strptime(text, date_format).replace(tzinfo=None)
        except ValueError:
            continue
...
def parse_season_number_from_href(href):
    match = re.search(r"/stagione(\d+)/", href, re.IGNORECASE)
    if match:
        return int(match.group(1))
    return None


def parse_visible_season_links(html_text, page_url):
    contexts = []
    seen = set()
    for href in re.findall(r'href=["\']([^"\']*episodi_[^"\']*sb\d+[^"\']*)', html_text, flags=re.I):
        href = html.unescape(href)
        match = re.search(r"(SE\d+),(ST\d+),sb(\d+)", href, flags=re.I)
        if not match:
            continue

        season_url = canonical_url(href)
        subbrand_id = match.group(3)
        if subbrand_id in seen:
            continue
        seen.add(subbrand_id)
        contexts.append(
            {
                "series_id": match.group(1),
                "season_id": match.group(2),
                "subbrand_id": subbrand_id,
                "season_number": parse_season_number_from_href(href),
    ...
    ids = extract_ids_from_url(page_url)
    if ids["subbrand_id"]:
        contexts.append(
            {
                "series_id": ids["series_id"],
                "season_id": ids["season_id"],
                "subbrand_id": ids["subbrand_id"],
                "season_number": parse_season_number_from_href(page_url),
                "season_url": ids["url"],
            }
        )
    return contexts
...
def fetch_series_data(series_id):
    ...
    payload = fetch_json(
        SERIES_FEED_URL,
        params={"byGuid": series_id, "form": "cjson", "httpError": "true"},
...
def visible_subbrand_contexts(input_url):
    ids = extract_ids_from_url(input_url)
    if ids["video_id"]:
        return []

    page_html = fetch_text(ids["url"], headers={"Accept": "text/html,*/*"})
    contexts = parse_visible_season_links(page_html, ids["url"])
    if ids["subbrand_id"]:
        return contexts

    series_data = fetch_series_data(ids["series_id"])
    if not series_data:
        return contexts

    slug = series_slug_from_url(input_url)
    season_lookup = {season.get("id"): season for season in series_data.get("seriesTvSeasons") or []}
    seen_subbrands = {context["subbrand_id"] for context in contexts}
    seen_seasons = {context["season_id"] for context in contexts if context.get("season_id")}

    for season_url in series_data.get("availableTvSeasonIds") or []:
        season = season_lookup.get(season_url) or {}
        season_number = parse_int(season.get("tvSeasonNumber"), 1)
        season_id = clean_text(season.get("guid"))
        if not slug or not season_id or season_id in seen_seasons:
            continue

        candidate_url = (
            f"{BASE_URL}/fiction/{slug}/stagione{season_number}/"
            f"episodi_{ids['series_id']},{season_id}"
        )
        try:
            season_html = fetch_text(candidate_url, headers={"Accept": "text/html,*/*"}, attempts=2)
        except Exception:
            continue

        for context in parse_visible_season_links(season_html, candidate_url):
            if context["subbrand_id"] in seen_subbrands:
                continue
            seen_subbrands.add(context["subbrand_id"])
            seen_seasons.add(context.get("season_id"))
            contexts.append(context)

    return contexts


def fetch_program_by_guid(guid):
    payload = fetch_json(
        f"{PROGRAMS_FEED_URL}/guid/-/{guid}",
...
def fetch_subbrand_entries(subbrand_id, limit=None):
    entries = []
    seen = set()
    start = 1
    page_size = 100
    ...
    while True:
        ...
        payload = fetch_json(
            PROGRAMS_FEED_URL,
            params={
                "byCustomValue": f"{{subBrandId}}{{{subbrand_id}}}",
        ...
        for entry in page_entries:
            guid = clean_text(entry.get("guid"))
...
def msi_publish_info(item):
    info = item.get("mediasetprogram$publishInfo") or {}
    return info if isinstance(info, dict) else {}


def msi_source_description(item):
    return clean_text(
        item.get("longDescription")
        or item.get("mediasetprogram$longDescriptionUI")
        or item.get("description")
        or "No Description"
    )


def msi_episode_number_from_item(item):
    episode = parse_int(item.get("tvSeasonEpisodeNumber") or item.get("mediasetprogram$episodeNumber"))
    if episode is not None:
        return episode

    match = re.search(r"\bepisodio\s*(\d+)", clean_text(item.get("title")), re.IGNORECASE)
    if match:
        return int(match.group(1))
    return None


def msi_video_url_from_item(item):
    video_url = canonical_url(item.get("mediasetprogram$videoPageUrl"))
    if not video_url and item.get("media"):
        video_url = canonical_url((item.get("media") or [{}])[0].get("publicUrl"))
    return video_url


def msi_metadata_from_item(item, context=None):
    context = context or {}
    season = parse_int(item.get("tvSeasonNumber"), context.get("season_number"))
    episode = msi_episode_number_from_item(item)
    title = clean_text(
        item.get("mediasetprogram$brandTitle")
        or item.get("mediasetprogram$auditelBrandName")
        or context.get("show_title")
        or "Mediaset Infinity IT"
    )
    episode_title = clean_text(item.get("title") or f"Episode {episode or ''}")
    published = clean_text(msi_publish_info(item).get("last_published") or item.get("updated"))
    ...
    return Metadata(
        ...
        description=msi_source_description(item),
        video_id=clean_text(item.get("guid")),
        year=parse_int((published or "")[:4]),
        content_type=clean_text(item.get("mediasetprogram$programType") or item.get("contentType")),
...
def extract_video_id(video_url):
    ...
    page_text = fetch_text(ids["url"], headers={"Accept": "text/html,*/*"})
    for pattern in (
        r'<meta\s+[^>]*name=["\']app-reference-id["\'][^>]*content=["\']([^"\']+)',
        r'<meta\s+[^>]*content=["\']([^"\']+)["\'][^>]*name=["\']app-reference-id["\']',
        r"[?&]programGuid=([^\"&]+)",
    ):
        match = re.search(pattern, page_text, flags=re.I)
        if match:
            return clean_text(match.group(1))
    raise ValueError("Could not find a Mediaset Infinity content ID for this URL.")


def search_metadata(video_url, video_id):
    item = fetch_program_by_guid(video_id)
    return msi_metadata_from_item(item)
...
def playback_error_message(payload):
    ...
    if code == "PL053":
        return "This content has no available purchasable rights."
    if "available" in message.lower():
        return "An Italian IP/VPN is required for this content."
...
def fetch_media_selector(video_id, auth):
    response = session.post(
        ...
        json={"contentId": video_id, "streamType": "VOD"},
        timeout=30,
    )
    response.raise_for_status()
...
def smil_params(media_selector, auth):
    params = {
        ...
        "assetTypes": media_selector.get("assetTypes"),
...
def parse_smil_media(smil_xml):
    ...
    for textstream in root.findall(".//{*}textstream"):
        ...
        language = SOURCE_LANGUAGE_CODE if raw_language.lower() in {"ita", "it", "italian"} else raw_language
        subtitles.append(
            {
                "url": sub_url,
                "language": language,
...
def is_episode_url(video_url):
    try:
        ...
        return False
    return bool(ids["video_id"] or "/video/" in urlparse(ids["url"]).path or "/movie/" in urlparse(ids["url"]).path)
...
def collect_episode_items(series_url, show_progress=True):
    if is_episode_url(series_url):
        raise ValueError("List/download mode requires a Mediaset Infinity series URL, not an episode/movie URL.")

    contexts = visible_subbrand_contexts(series_url)
    if not contexts:
        raise ValueError("Could not find a visible Mediaset Infinity episodes season/sub-brand.")

    items = []
    seen = set()
    for context in contexts:
        entries = fetch_subbrand_entries(context["subbrand_id"])
        if show_progress:
            season_label = f"Season {context['season_number']}" if context.get("season_number") else f"SubBrand {context['subbrand_id']}"
            print(f"{icons.ICON_INFO} {bcolors.LIGHTBLUE}{season_label}: {bcolors.ENDC}{len(entries)} episode(s)")

        for entry in entries:
            video_id = clean_text(entry.get("guid"))
            video_url = msi_video_url_from_item(entry)
            if not video_id or not video_url or video_id in seen:
                continue
            seen.add(video_id)
            metadata = msi_metadata_from_item(entry, context=context)
            items.append(
                EpisodeItem(
                    url=video_url,
                    title=metadata.title,
                    ...
                    video_id=video_id,
                    description=metadata.description,
                    air_date=clean_text(msi_publish_info(entry).get("last_published") or entry.get("updated")),
                )
            )

    if not items:
        raise ValueError("No Mediaset Infinity episodes found for this URL.")
...
def filename_episode_title(metadata):
    ...
    if episode is not None and re.fullmatch(rf"Episodio\s*0*{episode}", title, flags=re.IGNORECASE):
        return ""
    if episode is not None:
        title = re.sub(rf"^(?:Ep\.?|Episodio)\s*0*{episode}\s*[-:]\s*", "", title, flags=re.IGNORECASE)
        if not title:
            return ""
